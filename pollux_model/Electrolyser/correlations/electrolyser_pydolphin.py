...
class Electrolyser_pydolphin_DeGroot(Model):
    ...
    def calculate_output(self, u=0, x=0):
        result = self.calc_prod_rates(u,x)
    ...
    def calc_prod_rates(self,u,x):
        ...
        self.parameters['power_cell_real'] = power_input /self.parameters['N_cells'] #* self.power_multiplier  # todo: the power multiplier can be extended to include active and non active stacks, for now just give the independent stacks
        self.calc_i_cell()
        ### wteta faraday assume to be constant
        # Production rates [mol/s]
        #
        self.parameters['prod_rate_H2'] = (self.parameters['N_cells'] ) * self.I_cell_array / (2 * self.parameters['Faraday_const']) \
                            * self.parameters['eta_Faraday_array']
        self.parameters['prod_rate_O2'] = (self.parameters['N_cells']) * self.I_cell_array / (4 * self.parameters['Faraday_const']) \
                            * self.parameters['eta_Faraday_array']
        self.parameters['prod_rate_H2O'] = (self.parameters['N_cells'] ) * self.I_cell_array / (2 * self.parameters['Faraday_const'])
        ...
        self.parameters['flowrate_H2O'] = self.parameters['massflow_H2O'] / self.parameters['rho_H2O']
