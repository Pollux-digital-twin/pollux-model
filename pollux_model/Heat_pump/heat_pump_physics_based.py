...
class Heat_pump_nrel(Model):
    ...
    def __init__(self):
        ...
        self.yaml_file_path =os.path.join(self.script_dir,'NREL_components','heat_pump_model_inputs.yml')
    ...
    def initialize_state(self, x=0):
        ...
        #self.input_file_path = os.path.join(self.script_dir,'NREL_components','heat_pump_model_inputs.yml')
        self.heat_pump_component.construct_yaml_input_quantities(self.yaml_file_path)


    def update_state(self, u=0, x=0):
        """update the state based on input u and state x"""
        self.heat_pump_component.hot_temperature_desired = Q_(np.array([u['desired outlet hot temperature']] * self.parameters['n_hrs']), 'degC')
        self.heat_pump_component.hot_temperature_minimum = Q_(np.array([u['minimum outlet hot temperature']] * self.parameters['n_hrs']), 'degC')
        self.heat_pump_component.cold_temperature_available = Q_(np.array([u['available outlet cold temperature']] * self.parameters['n_hrs']), 'degC')
    ...
    def calculate_output(self, u=0, x=0):
        ...
        self.output['Hot mass flow rate(kg/s)'] = self.heat_pump_component.hot_mass_flowrate_average.m
...
if __name__ =='__main__':
    ...
    correlation_result = Heat_pump_nrel()
    # input_data = correlation_result.collect_user_inputs()
    correlation_result.initialize_state()
    print(correlation_result.parameters)
    input_file = {'desired outlet hot temperature':150,
                  'minimum outlet hot temperature':100,
                  'available outlet cold temperature':80,
                  'refrigerant':'n-Pentane',}

    correlation_result.update_state(input_file)
