...
if time_horizon % step_size_control != 0:
    raise ValueError(f"time_horizon ({time_horizon}) is not divisible by step_size_control ({step_size_control})")
...
time_vector_control = np.linspace(0, time_horizon, time_horizon//step_size_control + 1) #if using stairs instead of step for plotting
...
# splitter1_control = splitter1_control_profile(time_vector_control)
# step_function = StepFunction(splitter1_control, step_size_control)
step_function = StepFunction(zeros_array, step_size_control)
splitter1 = Splitter(step_function)

###testing (remove later_
# splitter1.input['input'] = 10
# splitter1.calculate_output()
# print(splitter1.output['output_0'])
# control2 = 2*control
# splitter1.update_time_function(control2)
# splitter1.calculate_output()
# print(splitter1.output['output_0'])
...
# splitter2_control = splitter2_control_profile(time_vector_control)
step_function = StepFunction(zeros_array, step_size_control)
...
### storage 
# hydrogen_storage_profile =  lambda t: 100/3600 * (t+1)/(t+1) # control variable kg/s: the hydrogen mass flow produced from the storage
# hydrogen_storage_control = hydrogen_storage_profile(time_vector_control)
# step_function = StepFunction(hydrogen_storage_control, step_size_control)
step_function = StepFunction(zeros_array, step_size_control)
hydrogen_storage = HydrogenTankModel(step_function)

param = dict()
param['timestep'] = np.diff(time_vector)[0]*3600 #3600  # 1 hour in seconds, should be taken equal to delta_t
...
# control_init_reshaped = control_init.reshape(len(components_with_control),-1)

# Here the controls of the components are updated with values provided by the json input file
...
if objective_name == "":
    ...
    bounds = [ (problem['controls']['lb'][ii]/scaling_factor[ii] , problem['controls']['ub'][ii]/scaling_factor[ii] ) for ii in range(len( problem['controls']['init']))]
    control_init_scaled = [ (control_init[ii]/scaling_factor[ii] ) for ii in range(len(control_init)) ]
    objective_label = problem["objective"] ["label"] # Just for plotting

    # testing (remove later)
    # objective = Objective(solver)
    # control1 = splitter1_control_profile(time_vector_control)
    # rmse1 = objective.kpi_rmse_power_demand(control1)
    # print(rmse1)
    # control2 = 1.1 * control1
    # rmse2 = objective.kpi_rmse_power_demand(control1)
    # print(rmse2)
    
    # objective function
    objective = Objective(solver, scaling_factor)
    objective_function = getattr(objective, objective_name) # Note: objective function is function of scaled control
    ...
    match method:
        case "trust-constr":
            ...
            result = minimize(objective_function, control_init_scaled, method = method, tol = 1E-6,
                              options={'maxiter' : problem["optimisation"]["maxiter"], 
                                       ...
                                       'finite_diff_rel_step' : problem["optimisation"]["finite_diff_step"], 
                                       }, 
        ...
        case "L-BFGS-B":
            ...
            result = minimize(objective_function, control_init_scaled, method = method,
                                options={'maxiter': problem["optimisation"]["maxiter"], 
                                        'verbose' : True, 'disp':True,
                                        'eps': problem["optimisation"]["finite_diff_step"],                    # absolute stepsize
                                        'ftol': 1E-16
                                        }, 
                                bounds = bounds, callback = call_back)
        case "Powell":
            # TNC does only do one iteration, not clear why. dont use it for now!
            # Powell has a reporting issue result.x does not reproduce reported minimal objective function value
            # below an attempt to repair it in the callback function. Powell seems however very efficient. 
            def call_back(x):
                function_value.append(objective_function(x))
        ...
        case "differential-evolution":
            ...
            result = differential_evolution(objective_function, maxiter = problem["optimisation"]["maxiter"],  bounds = bounds, callback = call_back)
    ...
    control = np.array([control_scaled[ii] * scaling_factor[ii] for ii in range(len(control_scaled))])
...
power_difference = [(power_demand[ii] - power_delivered[ii])/power_demand[ii] for ii in range(len(power_demand))]
...
hydrogen_difference = [(hydrogen_demand[ii] - hydrogen_delivered[ii])/hydrogen_demand[ii] for ii in range(len(hydrogen_demand))]
...
efficiency_electrolyser = [100 * conversion_factor_hydrogen * hydrogen_electrolyser_mass_flow_out[ii]/electrolyser_power_input[ii] for ii in range(len(electrolyser_power_input))]
##########################################################################
### Calculate KPIs
##########################################################################
...
#adding appels and peers: TODO convert kg/hr to MW=MJ/s.DONE
...
efficiency_electrolyser_total = 100 * hydrogen_electrolyser_mass_flow_out_sum/electrolyser_power_input_sum
...
ax1.stairs(control_reshaped[1], time_vector_control,  color='r', label=components_with_control[1], linestyle='--')
# ax1.step(time_vector_control, control_reshaped[0], color='r', where='post', label=components_with_control[0])
# ax1.step(time_vector_control, control_reshaped[1], color='r',  where='post', label=components_with_control[1], linestyle='--')
...
plt.grid(True)
ax2=ax1.twinx()
ax2.stairs(3600*control_reshaped[2], time_vector_control, color='b', label='Hydrogen Storage mass flow out')
# ax2.step(time_vector_control, 3600*control_reshaped[2], color='b',  where='post', label='Hydrogen Storage mass flow out')
...
plt.step(time_vector, electrolyser_power_input,  color='c',where='post', label='Electrolyser power input')
# sum = [x + y for x, y in zip(power_delivered, electrolyser_power_input)]
# plt.step(time_vector, sum, label='sum', linestyle='--') # just for checking, should be equal to power supply
...
### Hydrogen figure
fig = plt.figure(figsize=(10,6))
# plt.step(time_vector, hydrogen_electrolyser_mass_flow_out,  color='r', label='Hydrogen Electrolyser output')
plt.step(time_vector, hydrogen_electrolyser_to_demand, color='c', where='post', label='Hydrogen from Electrolyser to Demand')
plt.step(time_vector, hydrogen_electrolyser_to_storage, color='m',  where='post', label='Hydrogen from Electrolyser to Storage')
plt.step(time_vector, hydrogen_storage_mass_flow_out, color='y',where='post', label='Hydrogen from Storage to Demand' )
plt.step(time_vector, hydrogen_demand,  color='b', where='post', linewidth=2, label='Hydrogen Demand')
...
ax2.step(time_vector, hydrogen_storage_mass_flow_in, color = 'b', where='post', label='Hydrogen Storage mass flow in')
ax2.step(time_vector, hydrogen_storage_mass_flow_out, color = 'g', where='post', label='Hydrogen Storage mass flow out')
